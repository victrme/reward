...
function minerStats(json: type.MinerAPI) {
	...
	function countdown(d: type.RestantDate) {
	...
	function date(d: type.RestantDate) {
		...
		const addTenZero = (num: number): string =>
	...
	function history() {
		...
		json.payments.forEach((pay) => {
			const payDate = new Date(pay.timestamp * 1000)
			const p = document.createElement("p")

			p.innerText = payDate.toLocaleDateString("fr-fr", {
				day: "2-digit",
				month: "long",
				hour: "2-digit",
				minute: "2-digit",
	...
	id("minerLink").setAttribute(
		"href",
		`https://eth.2miners.com/account/${ADDRESS}`
	)
